...
-   Type you answer(s) in bullet points below
    ...
    ```{r}
    library(summarytools)dfSummary(penguins_data_from_csv)
    ```
...
```{r}
ggplot(penguins_data_from_csv, aes(x = culmen_depth_mm, fill = species)) +
  geom_histogram(color = "black") +
  theme_bw() +
  scale_fill_manual(values = c("lightpink", "hotpink", "deeppink")) +
  labs(
    title = "Distribution of Culmen Depth",
    subtitle = "Penguins by Species",
    x = "Culmen Depth (mm)",
    y = "Number of Penguins"
  )
```
...
```{r}
ggplot(penguins_data_from_csv, aes(x = culmen_depth_mm, y = body_mass_g, color = species)) +
  geom_point(size = 3, alpha = 0.9) +
  theme_bw() +
  scale_color_manual(values = c("hotpink", "limegreen", "dodgerblue")) +
  labs(
    title = "Penguin Culmen Depth vs. Body Mass",
    subtitle = "Custom Bright Colors by Species",
    x = "Culmen Depth (mm)",
    y = "Body Mass (g)",
    color = "Species"
  )
